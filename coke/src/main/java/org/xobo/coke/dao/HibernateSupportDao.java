...
public class HibernateSupportDao<K> extends HibernateDao {
  ...
  public <T> T findPropertyValue(Class<?> clazz, String uniqueProperty, Object value,
      String property) {
  ...
  public <T> Collection<T> load(Criteria criteria, Class<T> entityClass) {
    return load(null, null, null, criteria, entityClass);
  }


  public <T> Collection<T> load(Map<String, Object> queryParameter, Criteria criteria,
      Class<T> entityClass) {
    return load(null, queryParameter, null, criteria, entityClass);
  }

  public void load(Page<?> page, Map<String, Object> queryParameter, Criteria criteria,
      Class<?> entityClass) {
  ...
  public <T> Collection<T> load(Page<T> page, Map<String, Object> queryParameter,
    ...
    DetachedCriteria detachedCriteria =
        buildDetachedCriteria(queryParameter, propertyOperatorMap, criteria, entityClass);
    if (page != null) {
  ...
  public DetachedCriteria buildDetachedCriteria(Map<String, Object> queryParameter,
      Criteria criteria, Class<?> entityClass) {
  ...
  public DetachedCriteria buildDetachedCriteria(Map<String, Object> queryParameter,
      Map<String, PropertyWrapper> propertyOperatorMap, Criteria criteria, Class<?> entityClass) {
    criteria =
        parameterToCriteria.mergeQueryParameterCriteria(queryParameter, propertyOperatorMap,
  ...
  public void setPathValue(Session session, IBase<K> baseEntity, IBase<K> parent) {
    if (baseEntity instanceof PathModel) {
      ...
      if (indexNo == null) {
        indexNo =
            (Long) session.createCriteria(BeanReflectionUtils.getClass(baseEntity))
                .add(Restrictions.eq("parentId", parentId))
                .setProjection(Projections.max("indexNo")).uniqueResult();
      ...
      String newTypePath =
          parentTypePath + pathEntity.getType() + indexNo + PathModel.TypeSeparator;
  ...
  @SuppressWarnings("unchecked")
  private void recursiveBuildTypePath(Criteria criteria, Class<?> clazz, K parentId,
      String parentTypePath) {
